...
f = open('eval-lpwc-dt/graphsage/results/01_eval-graphsage-one-hot-encoding.txt', 'w')
sys.stdout = f
...
#load data
paper_csv = "lpwc/idx/nodes-nld/paper_nld.csv"
method_csv = "lpwc/idx/nodes-nld/method_nld.csv"
dataset_csv = "lpwc/idx/nodes-nld/dataset_nld.csv"
task_csv = "lpwc/idx/nodes-nld/task_nld.csv"
...
#Create a sparse one-hot encoded tensor
def sparse_one_hot(num_nodes):
    index_tensor = torch.stack((torch.arange(num_nodes), torch.arange(num_nodes)))
    values = torch.ones(num_nodes)
    return torch.sparse_coo_tensor(index_tensor, values, (num_nodes, num_nodes))


#Project the data to a lower dimension using a random matrix.
def random_projection_sparse(data, original_dim, target_dim):
    R = torch.randn((original_dim, target_dim))
    projected_data = torch.sparse.mm(data, R)
    return projected_data

# node types paper
data['paper'].x = sparse_one_hot(len(paper_df))

# node types method
data['method'].x = sparse_one_hot(len(method_df))

# node types dataset
data['dataset'].x = sparse_one_hot(len(dataset_df))

# node types task
data['task'].x = sparse_one_hot(len(task_df))

# Dimension for random projection
target_dim = 64 

# Apply random projection
data['paper'].x = random_projection_sparse(data['paper'].x, len(paper_df), target_dim)
data['method'].x = random_projection_sparse(data['method'].x, len(method_df), target_dim)
data['dataset'].x = random_projection_sparse(data['dataset'].x, len(dataset_df), target_dim)
data['task'].x = random_projection_sparse(data['task'].x, len(task_df), target_dim)
