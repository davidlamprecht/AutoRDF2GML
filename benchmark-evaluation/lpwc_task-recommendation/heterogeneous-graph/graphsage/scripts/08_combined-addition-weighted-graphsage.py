...
f = open('eval-lpwc-dt/graphsage/results/08_eval-graphsage-combined-addition-weighted.txt', 'w')
sys.stdout = f
...
#transe embeddings
paper_embeddings_csv = "lpwc/idx/nodes-transe/paper_transe.csv"
method_embeddings_csv = "lpwc/idx/nodes-transe/method_transe.csv"
...
paper_embeddings_df = pd.read_csv(paper_embeddings_csv, header=None)
method_embeddings_df = pd.read_csv(method_embeddings_csv, header=None)
...
paper_lin_literal = torch.nn.Linear(129, 64).eval()
method_lin_literal = torch.nn.Linear(130, 64).eval()
dataset_lin_literal = torch.nn.Linear(132, 64).eval()
task_lin_literal = torch.nn.Linear(128, 64).eval()

paper_tensor_literal = paper_lin_literal(torch.tensor(paper_df.values, dtype=torch.float))
method_tensor_literal = method_lin_literal(torch.tensor(method_df.values, dtype=torch.float))
dataset_tensor_literal = dataset_lin_literal(torch.tensor(dataset_df.values, dtype=torch.float))
task_tensor_literal = task_lin_literal(torch.tensor(task_df.values, dtype=torch.float))

# Linear Layers Embeddings
paper_lin_embedding = torch.nn.Linear(128, 64).eval()
method_lin_embedding = torch.nn.Linear(128, 64).eval()
dataset_lin_embedding = torch.nn.Linear(128, 64).eval()
task_lin_embedding = torch.nn.Linear(128, 64).eval()

paper_tensor_embedding = paper_lin_embedding(torch.tensor(paper_embeddings_df.values, dtype=torch.float))
method_tensor_embedding = method_lin_embedding(torch.tensor(method_embeddings_df.values, dtype=torch.float))
dataset_tensor_embedding = dataset_lin_embedding(torch.tensor(dataset_embeddings_df.values, dtype=torch.float))
task_tensor_embedding = task_lin_embedding(torch.tensor(task_embeddings_df.values, dtype=torch.float))


# addition of the tensors with weights
paper_tensor = literal_weight * paper_tensor_literal + embedding_weight * paper_tensor_embedding
method_tensor = literal_weight * method_tensor_literal + embedding_weight * method_tensor_embedding
dataset_tensor = literal_weight * dataset_tensor_literal + embedding_weight * dataset_tensor_embedding
task_tensor = literal_weight * task_tensor_literal + embedding_weight * task_tensor_embedding
...
class Model(torch.nn.Module):
    def __init__(self, hidden_channels):
        super().__init__()
        self.paper_lin = torch.nn.Linear(64, hidden_channels)
        self.method_lin = torch.nn.Linear(64, hidden_channels)
        self.dataset_lin = torch.nn.Linear(64, hidden_channels)
        self.task_lin = torch.nn.Linear(64, hidden_channels)
