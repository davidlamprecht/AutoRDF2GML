...
f = open('eval-lpwc-dt/hgt/results/06_eval-hgt-combined-concatenated..txt', 'w')
sys.stdout = f
...
combined_array_paper = np.concatenate([paper_df.values, paper_embeddings_df.values], axis=1)
combined_array_method = np.concatenate([method_df.values, method_embeddings_df.values], axis=1)
...
paper_tensor = torch.tensor(combined_array_paper, dtype=torch.float)
method_tensor = torch.tensor(combined_array_method, dtype=torch.float)
...
class Model(torch.nn.Module):
    def __init__(self, out_channels):
        super().__init__()
        self.paper_lin = torch.nn.Linear(257, out_channels)
        self.method_lin = torch.nn.Linear(258, out_channels)
        self.dataset_lin = torch.nn.Linear(260, out_channels)
        self.task_lin = torch.nn.Linear(256, out_channels)
