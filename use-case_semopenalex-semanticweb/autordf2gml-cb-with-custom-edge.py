...
config.read('/example/path/config.ini')
...
##########################################################################################
#
# START OPTIONAL CUSTOM EDGES
#
##########################################################################################


#author-author custom edge with features

#SPAQRL query for the custom edge
author_author = """
PREFIX dcterms: <http://purl.org/dc/terms/>

SELECT DISTINCT ?author  (COUNT (?author) as ?author_count)
WHERE {
    ?work dcterms:creator ?author_uri .
    ?work dcterms:creator ?author .
    FILTER (?author != ?author_uri)
}
GROUP BY ?author
"""

author_author_edges = []
author_author_edges_features = []


for author in nodes_data_pivoted_df["pivoted_df_author"]["subject"]:
    query_replace = author_author.replace("?author_uri", f"<{author}>")
    for row in graph.query(query_replace):
        edge = [author, row[0]]
        author_author_edges.append(edge)
        author_author_edges_features.append([row[1]])


#save custom edge and features

#edge list with RDF URIs
df = pd.DataFrame(author_author_edges)
df.to_csv('/example/path/custom-edges/edge_list_author_author_uris.csv', index=False, header=False)


#edge list with mapped IDs
for key_mapping, value_mapping in mapping_df.items():
    inverted_mapping = invert_mapping(value_mapping)
    df = df.astype(str)
    df = df.replace(inverted_mapping) 

df.to_csv('/example/path/custom-edges/edge_list_author_author.csv', index=False, header=False)


#custom edge feature list
df = pd.DataFrame(author_author_edges_features)
df.to_csv('/example/path/custom-edges/edge_feature_list_author_author.csv', index=False, header=False)

print("Done Custom Edges")

##########################################################################################
#
# END OPTIONAL CUSTOM EDGES
#
##########################################################################################
